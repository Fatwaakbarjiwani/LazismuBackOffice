...
function ModalAprove({ aprove, setAprove }) {
  ...
  const handleSave = () => {
    ...
    dispatch(getAproveCampaign(campaignCodeBefore));
  };
  ...
  return (
    <Modal show={aprove} onClose={() => setAprove(false)}>
      <Modal.Header>Aprove Campaign</Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <Button
          className="text-white bg-orange-500 active:scale-105"
          color={"none"}
          onClick={handleSave}
        >
          Aprove
        </Button>
        <Button color="gray" onClick={() => setAprove(false)}>
          Decline
        </Button>
      </Modal.Footer>
    </Modal>
...
ModalAprove.propTypes = {
  aprove: PropTypes.bool,
  setAprove: PropTypes.func,
  // id: PropTypes.string,
};
